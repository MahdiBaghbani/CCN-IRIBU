...
ccn_iribu_mkInterestObject(struct ccn_iribu_prefix_s *name, ccn_iribu_interest_opts_u *opts)
{
    ...
    i->pkt = (struct ccn_iribu_pkt_s *) ccn_iribu_calloc(1, sizeof(struct ccn_iribu_pkt_s));
    if (!i->pkt) {
        ccn_iribu_free(i);
        return NULL;
    }
...
ccn_iribu_mkSimpleInterest(struct ccn_iribu_prefix_s *name, ccn_iribu_interest_opts_u *opts)
{
    ...
    if (ccn_iribu_mkInterest(name, opts, tmp, tmp + CCN_IRIBU_MAX_PACKET_SIZE, &len, &offs)) {
        ccn_iribu_free(tmp);
        return NULL;
    }
...
#ifdef USE_SUITE_CCNB
        case CCN_IRIBU_SUITE_CCNB:
            ccn_iribu_ccnb_fillInterest(name, NULL, tmp, tmpend, CCN_IRIBU_MAX_PACKET_SIZE, len);
            (*offs) = 0;
            break;
#endif
#ifdef USE_SUITE_CCNTLV
                     ...
                     ccn_iribu_data_opts_u *opts)
{
    ...
    DEBUGMSG_CUTL(DEBUG, "mkSimpleContent (%s, %zu bytes)\n",
                  ccn_iribu_prefix_to_str(name, s, CCN_IRIBU_MAX_PREFIX_SIZE),
                  paylen);
...
#ifdef USE_SUITE_CCNB
        case CCN_IRIBU_SUITE_CCNB:
            ccn_iribu_ccnb_fillContent(name, payload, paylen, contentpos, tmp, tmp + *len, len);
            *offs = 0;
            break;
#endif
#ifdef USE_SUITE_CCNTLV
