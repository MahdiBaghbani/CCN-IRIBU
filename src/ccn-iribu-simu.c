...
void ccn_iribu_simu_add2cache(char node, const char *name, int seqn, void *data, int len)
{
...
void ccn_iribu_client_TX(char node, char *name, int seqn, int nonce)
{
    char tmp[512];
...
void ccn_iribu_simu_ethernet(void *dummy, void *dummy2)
{
    if (etherqueue) {
        int i, qlen;
        ...
        *pp = NULL;
        ...
        if (i < 5) {
            ...
            DEBUGMSG(DEBUG, "simu_ethernet: sending %d Bytes to %s, (qlen=%d)\n", p->len,
                     ll2ascii(p->dst, 6), qlen);

            ccn_iribu_core_RX(relays + i, 0, (unsigned char *) p->data, p->len, &sun.sa,
...
int ccn_iribu_app_RX(struct ccn_iribu_relay_s *ccn_iribu, struct ccn_iribu_content_s *c)
{
    ...
    ccn_iribu_simu_client_RX(ccnl, tmp, atoi(tmp2 + 1), (char *) c->pkt->content,
                             c->pkt->contlen);
    return 0;
}

void ccn_iribu_ll_TX(struct ccn_iribu_relay_s *relay, struct ccn_iribu_if_s *ifc,
                     sockunion *dst, struct ccn_iribu_buf_s *buf)
{
    ...
    DEBUGMSG(TRACE, "eth(simu)_ll_TX to %s len=%zd (qlen=%d) [0x%02x 0x%02x]\n",
             ccn_iribu_addr2ascii(dst), buf->datalen, cnt, buf->data[0], buf->data[1]);
...
void ccn_iribu_simu_init_node(char node, const char *addr, int max_cache_entries, int mtu)
{
    struct ccn_iribu_relay_s *relay = char2relay(node);
    ...
    if (node == 'A' || node == 'B') {
        ...
        struct ccn_iribu_client_s *client =
            ccn_iribu_calloc(1, sizeof(struct ccn_iribu_client_s));
        client->lastseq       = SIMU_NUMBER_OF_CHUNKS - 1;
        client->last_received = -1;
        client->name          = node == 'A' ? "/ccnl/simu/movie1" : "/ccnl/simu/movie2";
...
void ccn_iribu_simu_add_fwd(char node, const char *name, char dstnode, int mtu)
{
...
int ccn_iribu_simu_init(int max_cache_entries, int mtu)
{
    ...
    ccn_iribu_simu_init_node('A', "\x00\x00\x00\x00\x00\x0a", max_cache_entries, mtu);
    ccn_iribu_simu_init_node('B', "\x00\x00\x00\x00\x00\x0b", max_cache_entries, mtu);
    ccn_iribu_simu_init_node('C', "\x00\x00\x00\x00\x00\x0c", max_cache_entries, mtu);
    ccn_iribu_simu_init_node('1', "\x00\x00\x00\x00\x00\x01", max_cache_entries, mtu);
    ccn_iribu_simu_init_node('2', "\x00\x00\x00\x00\x00\x02", max_cache_entries, mtu);
