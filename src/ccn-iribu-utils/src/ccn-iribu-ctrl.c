...
size_t split_string(char *in, char c, char *out)
{
...
int8_t add_ccn_iribu_name(uint8_t *out, uint8_t *bufend, char *ccn_path, size_t *retlen)
{
    char comp[256];
    ...
    while ((h = split_string(ccn_path + len2, '/', comp))) {
        len2 += h;
        if (ccn_iribu_ccnb_mkStrBlob(out + len, bufend, CCN_DTAG_COMPONENT, CCN_TT_DTAG,
                                     comp, &len)) {
            return -1;
        }
        memset(comp, 0, 256);
...
int8_t mkDebugRequest(uint8_t *out, size_t buflen, char *dbg, char *private_key_path,
                      size_t *retlen)
{
    ...
    if (ccn_iribu_ccnb_mkHeader(out, out + buflen, CCN_DTAG_INTEREST, CCN_TT_DTAG,
                                &len)) {    // interest
        return -1;
    }
    if (ccn_iribu_ccnb_mkHeader(out + len, out + buflen, CCN_DTAG_NAME, CCN_TT_DTAG,
                                &len)) {    // name
        return -1;
    }

    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "ccnx", &len1)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "", &len1)) {
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "debug", &len1)) {
        return -1;
    }

    // prepare debug statement
    if (ccn_iribu_ccnb_mkHeader(stmt, stmt + sizeof(stmt), CCN_IRIBU_DTAG_DEBUGREQUEST,
                                CCN_TT_DTAG, &len3)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(stmt + len3, stmt + sizeof(stmt), CCN_DTAG_ACTION,
                                 CCN_TT_DTAG, "debug", &len3)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(stmt + len3, stmt + sizeof(stmt),
                                 CCN_IRIBU_DTAG_DEBUGACTION, CCN_TT_DTAG, dbg, &len3)) {
        return -1;
    }
    ...
    if (ccn_iribu_ccnb_mkHeader(contentobj, contentobj + sizeof(contentobj),
                                CCN_DTAG_CONTENTOBJ, CCN_TT_DTAG,
                                &len2)) {    // contentobj
        return -1;
    }
    if (ccn_iribu_ccnb_mkBlob(contentobj + len2, contentobj + sizeof(contentobj),
                              CCN_DTAG_CONTENT, CCN_TT_DTAG,    // content
                              (char *) stmt, len3, &len2)) {
        return -1;
    }
    ...
    if (ccn_iribu_ccnb_mkBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                              CCN_TT_DTAG,    // comp
...
int8_t mkNewEthDevRequest(uint8_t *out, size_t outlen, char *devname, char *ethtype,
                          char *frag, char *flags, char *private_key_path, size_t *reslen)
{
    ...
    if (ccn_iribu_ccnb_mkHeader(out, out + outlen, CCN_DTAG_INTEREST, CCN_TT_DTAG,
                                &len)) {    // interest
        return -1;
    }
    if (ccn_iribu_ccnb_mkHeader(out + len, out + outlen, CCN_DTAG_NAME, CCN_TT_DTAG,
                                &len)) {    // name
        return -1;
    }
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "newdev", &len1)) {
        return -1;
    }

    // prepare DEVINSTANCE
    if (ccn_iribu_ccnb_mkHeader(faceinst, faceinst + sizeof(faceinst),
                                CCN_IRIBU_DTAG_DEVINSTANCE, CCN_TT_DTAG, &len3)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, "newdev", &len3)) {
        return -1;
    }
    if (devname) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_DEVNAME, CCN_TT_DTAG, devname,
                                     &len3)) {
            return -1;
        }
    }
    if (ethtype) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_DTAG_PORT, CCN_TT_DTAG, ethtype, &len3)) {
            return -1;
        }
    }
    if (frag) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_FRAG, CCN_TT_DTAG, frag, &len3)) {
            return -1;
        }
    }
    if (flags) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_DEVFLAGS, CCN_TT_DTAG, flags,
                                     &len3)) {
            return -1;
        }
    }
...
int mkNewUDPDevRequest(uint8_t *out, size_t outlen, char *ip4src, char *ip6src,
                       char *port, char *frag, char *flags, char *private_key_path,
                       size_t *reslen)
{
    size_t len = 0, len1 = 0, len2 = 0, len3 = 0;
    uint8_t out1[CCN_IRIBU_MAX_PACKET_SIZE];
    ...
    if (ip4src) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_IP4SRC, CCN_TT_DTAG, ip4src, &len3)) {
            return -1;
        }
    }
    if (ip6src) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_IP6SRC, CCN_TT_DTAG, ip6src, &len3)) {
            return -1;
        }
    }
    if (port) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_DTAG_PORT, CCN_TT_DTAG, port, &len3)) {
            return -1;
        }
    }
    if (frag) {
...
int8_t mkDestroyDevRequest(uint8_t *out, size_t outlen, char *faceid,
...
int8_t mkEchoserverRequest(uint8_t *out, size_t outlen, char *path, int suite,
                           char *private_key_path, size_t *reslen)
{
    size_t len = 0, len1 = 0, len2 = 0, len3 = 0;
    uint8_t out1[CCN_IRIBU_MAX_PACKET_SIZE];
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out + outlen, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "ccnx", &len1)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out + outlen, CCN_DTAG_COMPONENT,
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out + outlen, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "echoserver", &len1)) {
        return -1;
    }

    // prepare FWDENTRY
    if (ccn_iribu_ccnb_mkHeader(fwdentry, fwdentry + sizeof(fwdentry),
                                CCN_DTAG_FWDINGENTRY, CCN_TT_DTAG, &len3)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(fwdentry + len3, fwdentry + sizeof(fwdentry),
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, "echoserver", &len3)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkHeader(fwdentry + len3, fwdentry + sizeof(fwdentry),
                                CCN_DTAG_NAME, CCN_TT_DTAG, &len3)) {    // prefix
        return -1;
    }
...
#endif
        if (ccn_iribu_ccnb_mkBlob(fwdentry + len3, fwdentry + sizeof(fwdentry),
                                  CCN_DTAG_COMPONENT, CCN_TT_DTAG, cp, cmplen, &len3)) {
            if (cp) {
                free(cp);
            }
            return -1;
    ...
    suite_s[0] = suite;
    if (ccn_iribu_ccnb_mkStrBlob(fwdentry + len3, fwdentry + sizeof(fwdentry),
                                 CCN_IRIBU_DTAG_SUITE, CCN_TT_DTAG, suite_s, &len3)) {
        return -1;
    }
    if (len3 >= sizeof(fwdentry)) {
        return -1;
    ...
    // prepare CONTENTOBJ with CONTENT
    if (ccn_iribu_ccnb_mkHeader(contentobj, contentobj + sizeof(contentobj),
                                CCN_DTAG_CONTENTOBJ, CCN_TT_DTAG, &len2)) {
        return -1;
    }    // contentobj
    if (ccn_iribu_ccnb_mkBlob(contentobj + len2, contentobj + sizeof(contentobj),
                              CCN_DTAG_CONTENT, CCN_TT_DTAG,    // content
...
int8_t mkNewFaceRequest(uint8_t *out, size_t outlen, char *macsrc, char *ip4src,
                        char *ip6src, char *wpan_addr, char *wpan_panid, char *host,
                        char *port, char *flags, char *private_key_path, size_t *reslen)
{
    size_t len = 0, len1 = 0, len2 = 0, len3 = 0;
    uint8_t out1[CCN_IRIBU_MAX_PACKET_SIZE];
    uint8_t contentobj[2000];
    ...
    if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, "newface", &len3)) {
        return -1;
    }
    if (macsrc) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_MACSRC, CCN_TT_DTAG, macsrc, &len)) {
            return -1;
        }
    }
    if (ip4src) {
        ...
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_DTAG_IPPROTO, CCN_TT_DTAG, "17", &len3)) {
            return -1;
        }
    }
    if (ip6src) {
    ...
    if (host) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_DTAG_HOST, CCN_TT_DTAG, host, &len3)) {
            return -1;
        }
    }
    if (port) {
    ...
    if (wpan_addr && wpan_panid) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_WPANADR, CCN_TT_DTAG, wpan_addr,
                                     &len3)) {
            return -1;
        }
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_WPANPANID, CCN_TT_DTAG, wpan_panid,
                                     &len3)) {
            return -1;
        }
    }
    ...
    if (frag) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst+len3, faceinst + sizeof(faceinst),
    CCN_IRIBU_DTAG_FRAG, CCN_TT_DTAG, frag, &len3)) { return -1;
        }
    }
    */
    if (flags) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_FACEFLAGS, CCN_TT_DTAG, flags,
                                     &len3)) {
            return -1;
        }
    }
...
int8_t mkNewUNIXFaceRequest(uint8_t *out, size_t outlen, char *path, char *flags,
                            char *private_key_path, size_t *reslen)
{
    size_t len = 0, len1 = 0, len2 = 0, len3 = 0;
    uint8_t out1[CCN_IRIBU_MAX_PACKET_SIZE];
    ...
    if (path) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                     CCN_IRIBU_DTAG_UNIXSRC, CCN_TT_DTAG, path, &len3)) {
            return -1;
        }
    }
    /*
...
int8_t mkDestroyFaceRequest(uint8_t *out, size_t outlen, char *faceid,
                            char *private_key_path, size_t *reslen)
{
    size_t len = 0, len1 = 0, len2 = 0, len3 = 0;
    uint8_t out1[CCN_IRIBU_MAX_PACKET_SIZE];
    ...
    if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, "destroyface", &len3)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                 CCN_DTAG_FACEID, CCN_TT_DTAG, faceid, &len3)) {
        return -1;
    }
    if (len3 >= sizeof(faceinst)) {
        return -1;
...
int8_t mkSetfragRequest(uint8_t *out, size_t outlen, char *faceid, char *frag, char *mtu,
                        char *private_key_path, size_t *reslen)
{
    size_t len = 0, len1 = 0, len2 = 0, len3 = 0;
    uint8_t out1[CCN_IRIBU_MAX_PACKET_SIZE];
    ...
    if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, "setfrag", &len3)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                 CCN_DTAG_FACEID, CCN_TT_DTAG, faceid, &len3)) {
    ...
    if (ccn_iribu_ccnb_mkStrBlob(faceinst + len3, faceinst + sizeof(faceinst),
                                 CCN_IRIBU_DTAG_MTU, CCN_TT_DTAG, mtu, &len3)) {
        return -1;
    }
    if (len3 >= sizeof(faceinst)) {
        return -1;
...
int8_t mkPrefixregRequest(uint8_t *out, size_t outlen, char reg, char *path, char *faceid,
                          int suite, char *private_key_path, size_t *reslen)
{
    size_t len = 0, len1 = 0, len2 = 0, len3 = 0;
    uint8_t out1[CCN_IRIBU_MAX_PACKET_SIZE];
    uint8_t contentobj[2000];
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, reg ? "prefixreg" : "prefixunreg", &len1)) {
        return -1;
    }
...
struct ccn_iribu_prefix_s *getPrefix(uint8_t *data, size_t datalen, int32_t *suite)
{
...
int8_t mkAddToRelayCacheRequest(uint8_t *out, size_t outlen, char *fname,
                                char *private_key_path, int *suite, size_t *reslen)
{
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + 5000, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "ccnx", &len1)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + 5000, CCN_DTAG_COMPONENT,
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + 5000, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "addcacheobject", &len1)) {
        goto Bail;
    }

    DEBUGMSG(DEBUG, "NAME:%s\n", prefix_string);

    if (ccn_iribu_ccnb_mkStrBlob(stmt + len3, stmt + 1000, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, prefix_string, &len3)) {
        goto Bail;
    }

    if (ccn_iribu_ccnb_mkHeader(contentobj + len2, contentobj + 4000, CCN_DTAG_CONTENTOBJ,
    ...
    if (ccn_iribu_ccnb_mkBlob(contentobj + len2, contentobj + 4000, CCN_IRIBU_DTAG_SUITE,
                              CCN_TT_DTAG,    // suite
    ...
    if (ccn_iribu_ccnb_mkBlob(contentobj + len2, contentobj + 4000,
                              CCN_IRIBU_DTAG_CHUNKNUM, CCN_TT_DTAG,    // chunknum
                              (char *) h, strlen((char *) h), &len2)) {
        goto Bail;
    }
    ...
    if (ccn_iribu_ccnb_mkBlob(contentobj + len2, contentobj + 4000,
                              CCN_IRIBU_DTAG_CHUNKFLAG, CCN_TT_DTAG,    // chunkflag
                              (char *) h, strlen((char *) h), &len2)) {
        goto Bail;
    }

    if (ccn_iribu_ccnb_mkBlob(contentobj + len2, contentobj + 4000, CCN_DTAG_NAME,
                              CCN_TT_DTAG,    // content
                              (char *) stmt, len3, &len2)) {
        goto Bail;
    }
    ...
    if (ccn_iribu_ccnb_mkBlob(out1 + len1, out1 + 5000, CCN_DTAG_COMPONENT,
                              CCN_TT_DTAG,    // comp
                              (char *) contentobj, len2, &len1)) {
        goto Bail;
    }
...
int8_t mkRemoveFormRelayCacheRequest(uint8_t *out, size_t outlen, char *ccn_path,
                                     char *private_key_path, size_t *reslen)
{
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "ccnx", &len1)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len1, out1 + sizeof(out1), CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "removecacheobject", &len1)) {
        return -1;
    }

    // prepare debug statement
    if (ccn_iribu_ccnb_mkHeader(stmt, stmt + sizeof(stmt), CCN_DTAG_CONTENT, CCN_TT_DTAG,
                                &len3)) {
        return -1;
    }
...
int udp_sendto2(int sock, char *address, uint16_t port, unsigned char *data, size_t len)
{
    ssize_t rc;
    ...
    rc = sendto(sock, data, len, 0, (struct sockaddr *) &si_other, sizeof(si_other));
    return rc;
}
...
int ux_sendto2(int sock, char *topath, unsigned char *data, size_t len)
{
    ...
    /* Send message. */
    rc =
        sendto(sock, data, len, 0, (struct sockaddr *) &name, sizeof(struct sockaddr_un));
    if (rc < 0) {
...
int8_t make_next_seg_debug_interest(int num, uint8_t *out, size_t outlen, size_t *reslen)
{
    size_t len = 0;
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out + len, out + outlen, CCN_DTAG_COMPONENT, CCN_TT_DTAG,
                                 "mgmt", &len)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out + len, out + outlen, CCN_DTAG_COMPONENT, CCN_TT_DTAG,
...
int8_t handle_ccn_signature(uint8_t **buf, size_t *buflen, char *relay_public_key)
{
    uint64_t num;
    uint8_t typ;
    int8_t verified  = 0;
 ...
 * @return
 */
int8_t check_has_next(uint8_t *buf, size_t len, char **recvbuffer, size_t *recvbufferlen,
                      char *relay_public_key, int8_t *verified)
{
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "hk:mp:v:u:x:")) != -1) {
        ...
        help:
            fprintf(stderr,
                    ...
                    "-m is a special mode which only prints the interest message of the "
    ...
    if (!strcmp(argv[1], "debug")) {
        ...
        if (mkNewEthDevRequest(out, sizeof(out), argv[2], argc > 3 ? argv[3] : "0x88b5",
                               argc > 4 ? argv[4] : "0", argc > 5 ? argv[5] : "0",
        ...
        if (mkNewUDPDevRequest(out, sizeof(out), argv[2], NULL,
                               argc > 3 ? argv[3] : "9695", argc > 4 ? argv[4] : "0",
        ...
        if (mkEchoserverRequest(out, sizeof(out), argv[2], suite, private_key_path,
                                &len)) {
            goto Bail;
        }
    } else if (!strcmp(argv[1], "newETHface") || !strcmp(argv[1], "newUDPface") ||
               !strcmp(argv[1], "newUDP6face")) {
        if (argc < 5) {
            goto help;
        }
        if (mkNewFaceRequest(
                out, sizeof(out), !strcmp(argv[1], "newETHface") ? argv[2] : NULL,
                !strcmp(argv[1], "newUDPface") ? argv[2] : NULL,
                !strcmp(argv[1], "newUDP6face") ? argv[2] : NULL, NULL, NULL, argv[3],
                argv[4], argc > 5 ? argv[5] : "0x0001", private_key_path, &len)) {
            goto Bail;
        }
        ...
        if (mkNewFaceRequest(out, sizeof(out), NULL, NULL, NULL, argv[2], argv[3], NULL,
                             NULL, argc > 5 ? argv[5] : "0x0001", private_key_path,
                             &len)) {
            goto Bail;
        }
        ...
        if (mkNewUNIXFaceRequest(out, sizeof(out), argv[2], argc > 3 ? argv[3] : "0x0001",
                                 private_key_path, &len)) {
            goto Bail;
        }
        ...
        if (mkSetfragRequest(out, sizeof(out), argv[2], argv[3], argv[4],
                             private_key_path, &len)) {
            goto Bail;
        }
        ...
        if (mkPrefixregRequest(out, sizeof(out), 1, argv[2], argv[3], suite,
                               private_key_path, &len)) {
            goto Bail;
        }
        ...
        if (mkPrefixregRequest(out, sizeof(out), 0, argv[2], argv[3], suite,
                               private_key_path, &len)) {
            goto Bail;
        }
        ...
        file_uri = argv[2];
        if (mkAddToRelayCacheRequest(out, sizeof(out), file_uri, private_key_path, &suite,
                                     &len)) {
            goto Bail;
        }
        ...
        ccn_path = argv[2];
        if (mkRemoveFormRelayCacheRequest(out, sizeof(out), ccn_path, private_key_path,
                                          &len)) {
            goto Bail;
        }
    } else {
    ...
    if (len > 0 && !msgOnly) {
        ...
        snprintf(mysockname, sizeof(mysockname), "/tmp/.ccn-light-ctrl-%d.sock",
                 getpid());
        ...
        hasNext = check_has_next(out, len, (char **) &recvbuffer, &recvbufferlen,
        ...
        while (hasNext) {
            ...
            if (make_next_seg_debug_interest(num++, interest2, sizeof(interest2),
                                             &len2)) {
                goto Bail;
            }
            if (!use_udp) {
            ...
            if (!use_udp) {
                ...
                ssize_t recvlen;
                recvlen =
                    recvfrom(sock, out, sizeof(out), 0, (struct sockaddr *) &si, &slen);
                if (recvlen < 0) {
                    goto Bail;
                }
                len = (size_t) recvlen;
        ...
        if (ccn_iribu_ccnb_mkHeader(recvbuffer2 + recvbufferlen2,
                                    recvbuffer2 + sizeof(char) * recvbufferlen + 1000,
        ...
        if (relay_public_key && use_udp) {
            ...
            if (verified) {
                snprintf(sigoutput, sizeof(sigoutput),
                         "All parts (%d) have been verified", numOfParts);
                if (ccn_iribu_ccnb_mkStrBlob(
                        recvbuffer2 + recvbufferlen2,
                        recvbuffer2 + sizeof(char) * recvbufferlen + 1000,
                ...
                snprintf(sigoutput, sizeof(sigoutput),
                         "NOT all parts (%d) have been verified", numOfParts);
                if (ccn_iribu_ccnb_mkStrBlob(
                        recvbuffer2 + recvbufferlen2,
                        recvbuffer2 + sizeof(char) * recvbufferlen + 1000,
                        CCN_DTAG_SIGNATURE, CCN_TT_DTAG, sigoutput, &recvbufferlen2)) {
