...
void ccn_iribu_hmac256_keyval(uint8_t *key, size_t klen,
...
void ccn_iribu_hmac256_keyid(uint8_t *key, size_t klen,
                                                    ...
                                                    size_t *retlen)
{
    ...
    *offset -= mdlength;    // reserve space for the digest
    mdoffset = *offset;
    if (ccn_iribu_ccntlv_prependTL(CCNX_TLV_TL_ValidationPayload, mdlength, offset,
                                   buf)) {
        return -1;
    }
    endofsign = *offset;
...
#            endif
    ...
    ccn_iribu_hmac256_sign(keyval, 64, buf + *offset, endofsign - *offset, buf + mdoffset,
                           &mdlength);
                                             ...
                                             size_t *offset, uint8_t *buf, size_t *reslen)
{
    ...
    *offset -= mdlength;    // sha256 msg digest bits, filled out later
    mdoffset = *offset;
    // mandatory
    if (ccn_iribu_ndntlv_prependTL(NDN_TLV_SignatureValue, (uint64_t) mdlength, offset,
                                   buf)) {
        return -1;
    }
...
#            endif
    ...
    if (ccn_iribu_ndntlv_prependTL(NDN_TLV_SignatureInfo, endofsign - *offset, offset,
                                   buf)) {
        return -1;
    }

    // mandatory payload/content
    if (ccn_iribu_ndntlv_prependBlob(NDN_TLV_Content, payload, paylen, offset, buf)) {
        return -1;
    }
    ...
    if (final_block_id) {
        if (ccn_iribu_ndntlv_prependIncludedNonNegInt(
                NDN_TLV_NameComponent, *final_block_id, NDN_Marker_SegmentNumber, offset,
                buf)) {
            return -1;
        }
        // optional
        if (ccn_iribu_ndntlv_prependTL(NDN_TLV_FinalBlockId, oldoffset2 - *offset, offset,
                                       buf)) {
            return -1;
        }
    }
    ...
    // mandatory
    if (ccn_iribu_ndntlv_prependTL(NDN_TLV_Data, oldoffset - *offset, offset, buf)) {
        return -1;
    }
