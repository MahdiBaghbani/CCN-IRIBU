                                ...
                                struct ccn_iribu_pkt_s **pkt)
{
    ...
    if (from) {
        ...
        if (from_as_str) {
            DEBUGMSG_CFWD(
                INFO, "  incoming data=<%s>%s from=%s\n",
                ccn_iribu_prefix_to_str((*pkt)->pfx, s, CCN_IRIBU_MAX_PREFIX_SIZE),
                ccn_iribu_suite2str((*pkt)->suite), from_as_str ? from_as_str : "");
        }
    } else {
        DEBUGMSG_CFWD(INFO, "  incoming data=<%s>%s from=%s\n",
                      ccn_iribu_prefix_to_str((*pkt)->pfx, s, CCN_IRIBU_MAX_PREFIX_SIZE),
                      ccn_iribu_suite2str((*pkt)->suite), "");
    }
    ...
    if (pkt->pfx->compcnt == 2 && !memcmp(pkt->pfx->comp[0], "ccnx", 4) &&
        !memcmp(pkt->pfx->comp[1], "crypto", 6) && from == relay->crypto_face) {
...
#ifdef USE_FRAG
// returning 0 if packet was
int ccn_iribu_fwd_handleFragment(struct ccn_iribu_relay_s *relay,
                                 struct ccn_iribu_face_s *from,
                                 struct ccn_iribu_pkt_s **pkt, dispatchFct callback)
{
    ...
    ccn_iribu_frag_RX_BeginEnd2015(callback, relay, from, relay->ifs[from->ifndx].mtu,
                                   ((*pkt)->flags >> 2) & 0x03, (*pkt)->val.seqno, &data,
                                   &datalen);
...
int ccn_iribu_pkt_fwdOK(struct ccn_iribu_pkt_s *pkt)
{
    switch (pkt->suite) {
...
#    endif
    ...
    if ((*pkt)->suite == CCN_IRIBU_SUITE_CCNB && (*pkt)->pfx->compcnt == 4 &&
        ...
        ccn_iribu_mgmt(relay, (*pkt)->buf, (*pkt)->pfx,
                       from);    // use return value? // TODO uncomment
        return 0;
    }
#endif
...
#    endif
    ...
    if (!i) {
        ...
        DEBUGMSG_CFWD(DEBUG, "  created new interest entry %p (prefix=%s)\n", (void *) i,
                      ccn_iribu_prefix_to_str(i->pkt->pfx, s, CCN_IRIBU_MAX_PACKET_SIZE));
    }
...
#        ifdef USE_FRAG
        ...
        case CCN_IRIBU_DTAG_FRAGMENT2012: {
            ...
            dlen = (int) *datalen;
            rc   = ccn_iribu_frag_RX_frag2012(ccn_iribu_ccnb_forwarder, relay, from, data,
                                            &dlen);
            if (dlen < 0) {
                return -1;
            }
        ...
        case CCN_IRIBU_DTAG_FRAGMENT2013: {
            ...
            dlen = (int) *datalen;
            rc   = ccn_iribu_frag_RX_CCNx2013(ccn_iribu_ccnb_forwarder, relay, from, data,
                                            &dlen);
            if (dlen < 0) {
                return -1;
            }
...
int ccn_iribu_set_tap(struct ccn_iribu_relay_s *relay, struct ccn_iribu_prefix_s *pfx,
