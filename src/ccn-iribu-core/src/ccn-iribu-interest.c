                                                    ...
                                                    struct ccn_iribu_pkt_s **pkt)
{
    ...
    DEBUGMSG_CORE(TRACE, "ccn_iribu_new_interest(prefix=%s, suite=%s)\n",
...
int ccn_iribu_interest_isSame(struct ccn_iribu_interest_s *i, struct ccn_iribu_pkt_s *pkt)
{
    if (i) {
        if (pkt) {
            if (i->pkt->pfx->suite != pkt->suite ||
                ccn_iribu_prefix_cmp(i->pkt->pfx, NULL, pkt->pfx, CMP_EXACT)) {
                return 0;
            }
    ...
    if (i) {
        ...
        if (from) {
            ...
            for (pi = i->pending; pi; pi = pi->next) {    // check whether already listed
                if (pi->face == from) {
                    DEBUGMSG_CORE(DEBUG,
                                  "  we found a matching interest, updating time\n");
                    pi->last_used = CCN_IRIBU_NOW();
                    return 0;
                }
                last = pi;
            }
            pi = (struct ccn_iribu_pendint_s *) ccn_iribu_calloc(
                1, sizeof(struct ccn_iribu_pendint_s));
            if (!pi) {
            ...
            DEBUGMSG_CORE(
                DEBUG, "  appending a new pendint entry %p <%s>(%p)\n", (void *) pi,
...
int ccn_iribu_interest_remove_pending(struct ccn_iribu_interest_s *interest,
            ...
            while (pend) {    // TODO: is this really the most elegant solution?
                if (face->faceid == pend->face->faceid) {
                    DEBUGMSG_CFWD(INFO, "  removed face (%s) for interest %s\n",
                                  ...
                                  ccn_iribu_prefix_to_str(interest->pkt->pfx, s,
