...
// sa!=NULL && ifndx!=-1: use this (incoming) interface for outgoing
{
    ...
    if (ifndx >= 0) {
        if (ccn_iribu->defaultFaceScheduler) {
            f->sched = ccn_iribu->defaultFaceScheduler(
                ccn_iribu, (void (*)(void *, void *)) ccn_iribu_face_CTS);
        }
    ...
    if (ifc) {
        ...
        if (buf) {
            DEBUGMSG_CORE(TRACE, "enqueue interface=%p buf=%p len=%zu (qlen=%zu)\n",
                          (void *) ifc, (void *) buf, buf ? buf->datalen : 0,
                          ifc ? ifc->qlen : 0);
        }
...
void ccn_iribu_face_CTS(struct ccn_iribu_relay_s *ccn_iribu, struct ccn_iribu_face_s *f)
{
    ...
    if (!f->frag || f->frag->protocol == CCN_IRIBU_FRAG_NONE) {
        ...
        if (buf) {
            ccn_iribu_interface_enqueue(ccn_iribu_face_CTS_done, f, ccn_iribu,
                                        ccn_iribu->ifs + f->ifndx, buf, &f->peer);
        }
    }
#ifdef USE_FRAG
    else {
                           ...
                           struct ccn_iribu_face_s *to, struct ccn_iribu_buf_s *buf)
{
    ...
    DEBUGMSG_CORE(TRACE, "enqueue face=%p (id=%d.%d) buf=%p len=%zd\n", (void *) to,
                  ccn_iribu->id, to->faceid, (void *) buf, buf ? buf->datalen : 0);
                                  ...
                                  struct ccn_iribu_interest_s *i)
{
    ...
    for (fwd = ccn_iribu->fib; fwd; fwd = fwd->next) {
        ...
        if (!i->from || fwd->face != i->from ||
            ...
            DEBUGMSG_CFWD(
                INFO, "  outgoing interest=<%s> nonce=%i to=%s\n",
                ccn_iribu_prefix_to_str(i->pkt->pfx, s, CCN_IRIBU_MAX_PREFIX_SIZE), nonce,
                fwd->face ? ccn_iribu_addr2ascii(&fwd->face->peer) : "<tap>");
...
#    if !(defined(__FreeBSD__) || defined(__APPLE__))
        case (AF_PACKET): {
            ...
            fibface = ccn_iribu_get_face_or_create(ccn_iribu, i, &sun.sa,
...
#    endif
        ...
        case (AF_IEEE802154): {
            ...
            fibface =
                ccn_iribu_get_face_or_create(ccn_iribu, i, &sun.sa, sizeof(sun.wpan));
            break;
        }
#endif
#ifdef USE_IPV4
        case (AF_INET):
            ...
            fibface =
                ccn_iribu_get_face_or_create(ccn_iribu, i, &sun.sa, sizeof(sun.ip4));
            break;
#endif
#ifdef USE_IPV6
        case (AF_INET6):
            ...
            fibface =
                ccn_iribu_get_face_or_create(ccn_iribu, i, &sun.sa, sizeof(sun.ip6));
            break;
#endif
        }
        if (fibface) {
                            ...
                            struct ccn_iribu_content_s *c)
{
    ...
    DEBUGMSG_CORE(DEBUG, "ccn_iribu_content_add2cache (%d/%d) --> %p = %s [%d]\n",
                  ccn_iribu->contentcnt, ccn_iribu->max_cache_entries, (void *) c,
    ...
    if (ccn_iribu->max_cache_entries > 0 &&
        ...
        !cache_strategy_remove(ccn_iribu, c)) {
...
#ifndef CCN_IRIBU_LINUXKERNEL
                DEBUGMSG_CFWD(
                    INFO, "  outgoing data=<%s>%s nonce=%" PRIi32 " to=%s\n",
...
#else
                DEBUGMSG_CFWD(
                    INFO, "  outgoing data=<%s>%s nonce=%d to=%s\n",
                    ccn_iribu_prefix_to_str(i->pkt->pfx, s, CCN_IRIBU_MAX_PREFIX_SIZE),
                    ccn_iribu_suite2str(i->pkt->pfx->suite), nonce,
                    ccn_iribu_addr2ascii(&pi->face->peer));
#endif
...
void ccn_iribu_do_ageing(void *ptr, void *dummy)
{
    ...
    while (c) {
        if ((c->last_used + CCN_IRIBU_CONTENT_TIMEOUT) <= (uint32_t) t &&
            ...
            if (c->pkt->suite == CCN_IRIBU_SUITE_NDNTLV) {
                // Mark content as stale if its freshness period expired and it is not
                // static
                if ((c->last_used + (c->pkt->s.ndntlv.freshnessperiod / 1000)) <=
                        (uint32_t) t &&
                    !(c->flags & CCN_IRIBU_CONTENT_FLAGS_STATIC)) {
    ...
    while (i) {    // CONFORM: "Entries in the PIT MUST timeout rather
        ...
        if ((i->last_used + i->lifetime) <= (uint32_t) t ||
            ...
            DEBUGMSG_AGEING("AGING: REMOVE INTEREST", "timeout: remove interest", s,
...
int ccn_iribu_fib_add_entry(struct ccn_iribu_relay_s *relay,
                            struct ccn_iribu_prefix_s *pfx, struct ccn_iribu_face_s *face)
{
