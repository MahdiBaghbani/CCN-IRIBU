...
int8_t ccn_iribu_mgmt_parse_eth_address(uint8_t *sll_addr, const char *str)
{
    char *endptr;
...
int8_t ccn_iribu_mgmt_send_return_split(struct ccn_iribu_relay_s *ccn_iribu,
                                        struct ccn_iribu_buf_s *orig,
                                        struct ccn_iribu_prefix_s *prefix,
                                        struct ccn_iribu_face_s *from, size_t len,
                                        uint8_t *buf)
{
    ...
    for (it = 0; it < numPackets; ++it) {
        ...
        if (ccn_iribu_ccnb_mkHeader(packet + len4, packetend, CCN_IRIBU_DTAG_FRAG,
                                    CCN_TT_DTAG, &len4)) {
            goto Bail;
        }
        if (it == numPackets - 1) {
            if (ccn_iribu_ccnb_mkStrBlob(packet + len4, packetend, CCN_DTAG_ANY,
                                         CCN_TT_DTAG, "last", &len4)) {
                goto Bail;
            }
        }
        ...
        if (ccn_iribu_ccnb_mkBlob(packet + len4, packetend, CCN_DTAG_CONTENTDIGEST,
                                  CCN_TT_DTAG, (char *) buf + it * packetsize, len5,
                                  &len4)) {
            goto Bail;
        }
...
#    ifdef USE_SIGNATURES
        if (!ccn_iribu_is_local_addr(&from->peer)) {
            //                ccn_iribu_crypto_sign(ccn_iribu, packet, len4,
            //                "ccn_iribu_mgmt_crypto", id);
...
#    endif
            ...
            len5 = 0;
            if (ccn_iribu_ccnb_mkHeader(buf2, buf2end, CCN_DTAG_CONTENTOBJ, CCN_TT_DTAG,
                                        &len5)) {    // content
                goto Bail;
            }
            ...
            if (it == 0) {
                ...
                pkt->buf =
                    ccn_iribu_mkSimpleContent(pkt->pfx, buf2, len5, &contentpos, NULL);
                if (!pkt->buf) {
                    goto Bail;
                }
                ...
                /*
                                ...
                                unsigned char *ht = (unsigned char *)
                   ccn_iribu_malloc(sizeof(char)*20); int contlen; pkt =
                   ccn_iribu_ccnb_extract(&cp, &len5, 0, 0, 0, 0, &prefix_a, NULL, NULL,
                   &content, &contlen);
                   ...
                   ccn_iribu_prefix_to_path(prefix_a)); prefix_a->compcnt = 2;
                                prefix_a->comp = (unsigned char **)
                   ccn_iribu_malloc(sizeof(unsigned char*)*2); prefix_a->comp[0] =
                   (unsigned char *)"mgmt"; sprintf((char*)ht, "seqnum-%d", it);
                                prefix_a->comp[1] = ht;
                                prefix_a->complen = (int *)
                   ccn_iribu_malloc(sizeof(int)*2); prefix_a->complen[0] = strlen("mgmt");
                                prefix_a->complen[1] = strlen((char*)ht);
                                c = ccn_iribu_content_new(ccn_iribu, CCN_IRIBU_SUITE_CCNB,
                   &pkt, &prefix_a, NULL, content, contlen);
                                //if (!c) goto Done;
...
ccn_iribu_prefix_clone(struct ccn_iribu_prefix_s *p)
{
    ...
    p2 = (struct ccn_iribu_prefix_s*) ccn_iribu_calloc(1, sizeof(struct
ccn_iribu_prefix_s)); if (!p2) return NULL; for (i = 0, len = 0; i < p->compcnt; len +=
p->complen[i++]); p2->bytes = (unsigned char*) ccn_iribu_malloc(len); p2->comp = (unsigned
char**) ccn_iribu_malloc(p->compcnt*sizeof(char *)); p2->complen = (int*)
ccn_iribu_malloc(p->compcnt*sizeof(int)); if (!p2->comp || !p2->complen || !p2->bytes)
goto Bail; p2->compcnt = p->compcnt; for (i = 0, len = 0; i < p->compcnt; len +=
p2->complen[i++]) { p2->complen[i] = p->complen[i]; p2->comp[i] = p2->bytes + len;
...
#    define extractStr(VAR, DTAG)                                                    \
        if (typ == CCN_TT_DTAG && num == DTAG) {                                     \
            ...
            uint8_t *valptr;                                                         \
            size_t vallen;                                                           \
            if (ccn_iribu_ccnb_consume(typ, num, &buf, &buflen, &valptr, &vallen)) { \
                goto Bail;                                                           \
            }                                                                        \
            s = ccn_iribu_malloc(vallen + 1);                                        \
            if (!s) {                                                                \
                goto Bail;                                                           \
            }                                                                        \
...
int8_t ccn_iribu_mgmt_return_ccn_msg(struct ccn_iribu_relay_s *ccn_iribu,
                                     struct ccn_iribu_buf_s *orig,
                                     struct ccn_iribu_prefix_s *prefix,
                                     struct ccn_iribu_face_s *from, char *component_type,
                                     char *answer)
{
    size_t len = 0, len3 = 0;

    if (ccn_iribu_ccnb_mkHeader(out1 + len, out1 + OUT1_SIZE, CCN_DTAG_NAME, CCN_TT_DTAG,
                                &len)) {
        return -1;
    }

    if (ccn_iribu_ccnb_mkStrBlob(out1 + len, out1 + OUT1_SIZE, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "ccnx", &len)) {
        return -1;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len, out1 + OUT1_SIZE, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "", &len)) {
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out1 + len, out1 + OUT1_SIZE, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, component_type, &len)) {
        return -1;
    }
    if (len + 1 >= OUT1_SIZE) {
        return -1;
    ...
    // prepare FWDENTRY
    if (ccn_iribu_ccnb_mkStrBlob(out3, out3 + OUT3_SIZE, CCN_DTAG_ACTION, CCN_TT_DTAG,
                                 answer, &len3)) {
        return -1;
    }

    if (ccn_iribu_ccnb_mkBlob(out1 + len, out1 + OUT1_SIZE, CCN_DTAG_CONTENT,
                              CCN_TT_DTAG,    // content
                              (char *) out3, len3, &len)) {
        return -1;
    }

    if (ccn_iribu_mgmt_send_return_split(ccn_iribu, orig, prefix, from, len,
                                         (unsigned char *) out1)) {
        return -1;
    }
    return 0;
...
static int8_t
ccn_iribu_mgmt_create_interface_stmt(size_t num_interfaces, int *interfaceifndx,
                                     long *interfacedev, int *interfacedevtype,
                                     int *interfacereflect, char **interfaceaddr,
                                     uint8_t *stmt, const uint8_t *stmtend, size_t *len3)
{
    ...
    for (it = 0; it < num_interfaces; ++it) {    // interface content
        if (ccn_iribu_ccnb_mkHeader(stmt + *len3, stmtend, CCN_IRIBU_DTAG_INTERFACE,
                                    CCN_TT_DTAG, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_IFNDX,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_ADDRESS,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (interfacedevtype[it] == 1) {
            ...
            if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_ETH,
                                         CCN_TT_DTAG, str, len3)) {
                return -1;
            }
        } else if (interfacedevtype[it] == 2) {
            snprintf(str, sizeof(str), "%p", (void *) interfacedev[it]);
            if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_SOCK,
                                         CCN_TT_DTAG, str, len3)) {
                return -1;
            }
        } else {
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_REFLECT,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
...
static int8_t ccn_iribu_mgmt_create_faces_stmt(size_t num_faces, int *faceid,
                                               long *facenext, long *faceprev,
                                               int *faceifndx, int *faceflags,
                                               int *facetype, char **facepeer,
                                               char **facefrag, unsigned char *stmt,
                                               const uint8_t *stmtend, size_t *len3)
{
    size_t it;
    char str[100];
    (void) facefrag;
    for (it = 0; it < num_faces; ++it) {    // FACES CONTENT
        if (ccn_iribu_ccnb_mkHeader(stmt + *len3, stmtend, CCN_DTAG_FACEINSTANCE,
                                    CCN_TT_DTAG, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_DTAG_FACEID, CCN_TT_DTAG,
                                     str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_NEXT,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_PREV,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_FACEFLAGS,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (facetype[it] == AF_INET) {
            if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_IP,
                                         CCN_TT_DTAG, facepeer[it], len3)) {
                return -1;
            }
#    ifdef USE_LINKLAYER
...
#    endif
            ...
            if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_UNIX,
                                         CCN_TT_DTAG, facepeer[it], len3)) {
                return -1;
            }
        } else {
            snprintf(str, sizeof(str), "peer=?");
            if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_PEER,
                                         CCN_TT_DTAG, str, len3)) {
                return -1;
            }
        }
...
static int8_t ccn_iribu_mgmt_create_fwds_stmt(size_t num_fwds, long *fwd, long *fwdnext,
                                              long *fwdface, int *fwdfaceid, int *suite,
                                              int *fwdprefixlen, char **fwdprefix,
                                              uint8_t *stmt, const uint8_t *stmtend,
                                              size_t *len3)
{
    size_t it;
    char str[100];
    ...
    for (it = 0; it < num_fwds; ++it) {    // FWDS content
        if (ccn_iribu_ccnb_mkHeader(stmt + *len3, stmtend, CCN_DTAG_FWDINGENTRY,
                                    CCN_TT_DTAG, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_FWD,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_FACE,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_SUITE,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }

        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_PREFIX,
                                     CCN_TT_DTAG, fwdprefix[it], len3)) {
            return -1;
        }
...
static int8_t ccn_iribu_mgmt_create_interest_stmt(
    size_t num_interests, long *interest, long *interestnext, long *interestprev,
    int *interestlast, int *interestmin, int *interestmax, int *interestretries,
    long *interestpublisher, int *interestprefixlen, char **interestprefix, uint8_t *stmt,
    const uint8_t *stmtend, size_t *len3)
{
    size_t it;
    char str[100];
    ...
    for (it = 0; it < num_interests; ++it) {    // interest content
        if (ccn_iribu_ccnb_mkHeader(stmt + *len3, stmtend, CCN_DTAG_INTEREST, CCN_TT_DTAG,
                                    len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_INTERESTPTR,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_LAST,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_MIN,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_MAX,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_RETRIES,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_PUBLISHER,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }

        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_PREFIX,
                                     CCN_TT_DTAG, interestprefix[it], len3)) {
            return -1;
        }
...
static int8_t ccn_iribu_mgmt_create_content_stmt(size_t num_contents, long *content,
                                                 long *contentnext, long *contentprev,
                                                 int *contentlast_use,
                                                 int *contentserved_cnt, char **ccontents,
                                                 char **cprefix, uint8_t *stmt,
                                                 const uint8_t *stmtend, size_t *len3)
{
    size_t it;
    char str[100];
    ...
    for (it = 0; it < num_contents; ++it) {    // content content
        if (ccn_iribu_ccnb_mkHeader(stmt + *len3, stmtend, CCN_DTAG_CONTENT, CCN_TT_DTAG,
                                    len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_CONTENTPTR,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_LASTUSE,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }
        ...
        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_SERVEDCTN,
                                     CCN_TT_DTAG, str, len3)) {
            return -1;
        }

        if (ccn_iribu_ccnb_mkStrBlob(stmt + *len3, stmtend, CCN_IRIBU_DTAG_PREFIX,
                                     CCN_TT_DTAG, cprefix[it], len3)) {
            return -1;
        }
...
int8_t ccn_iribu_mgmt_debug(struct ccn_iribu_relay_s *ccn_iribu,
                            struct ccn_iribu_buf_s *orig,
                            struct ccn_iribu_prefix_s *prefix,
                            struct ccn_iribu_face_s *from)
{
    ...
    int *faceid = NULL, *faceifndx = NULL, *faceflags = NULL,
        *facetype  = NULL;    // store face-info
    ...
    int *interestlast = NULL, *interestmin = NULL, *interestmax = NULL,
        *interestretries = NULL, *interestprefixlen = NULL;
    long *interest = NULL, *interestnext = NULL, *interestprev = NULL,
         *interestpublisher = NULL;
    ...
    size_t num_faces = 0, num_fwds = 0, num_interfaces = 0, num_interests = 0,
           num_contents = 0, buflen = 0;
    uint64_t num;
    uint8_t typ;
    ...
    if (debugaction) {
        ...
        if (!strcmp((char *) debugaction, "dump")) {
            ...
            get_faces_dump(0, ccn_iribu, faceid, facenext, faceprev, faceifndx, faceflags,
                           facepeer, facetype, facefrag);
            get_fwd_dump(0, ccn_iribu, fwd, fwdnext, fwdface, fwdfaceid, suite,
                         fwdprefixlen, fwdprefix);
            get_interface_dump(0, ccn_iribu, interfaceifndx, interfaceaddr, interfacedev,
                               interfacedevtype, interfacereflect);
            get_interest_dump(0, ccn_iribu, interest, interestnext, interestprev,
                              interestlast, interestmin, interestmax, interestretries,
                              interestpublisher, interestprefixlen, interestprefix);
...
SoftBail:
    ...
    stmt_length = 200 * num_faces + 200 * num_interfaces +
                  200 * num_fwds    // alloc stroage for answer dynamically.
    ...
    if (ccn_iribu_ccnb_mkHeader(out + len, out + object_length, CCN_DTAG_NAME,
                                CCN_TT_DTAG, &len)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out + len, out + object_length, CCN_DTAG_COMPONENT,
                                 CCN_TT_DTAG, "ccnx", &len)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out + len, out + object_length, CCN_DTAG_COMPONENT,
    ...
    if (ccn_iribu_ccnb_mkHeader(stmt, stmt + stmt_length, CCN_IRIBU_DTAG_DEBUGREQUEST,
                                CCN_TT_DTAG, &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(stmt + len3, stmt + stmt_length, CCN_DTAG_ACTION,
                                 CCN_TT_DTAG, (char *) debugaction, &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(stmt + len3, stmt + stmt_length,
                                 CCN_IRIBU_DTAG_DEBUGACTION, CCN_TT_DTAG, cp, &len3)) {
        goto Bail;
    }
    ...
    if (!strcmp((char *) debugaction, "dump") ||
        !strcmp((char *) debugaction, "dump+halt")) {    // halt returns no content
        if (ccn_iribu_ccnb_mkHeader(stmt + len3, stmt + stmt_length,
                                    CCN_IRIBU_DTAG_DEBUGREPLY, CCN_TT_DTAG, &len3)) {
            goto Bail;
        }
        // len3 += ccn_iribu_ccnb_mkStrBlob(stmt+len3, CCN_IRIBU_DTAG_PREFIX, CCN_TT_DTAG,
        // cinterfaces[it]);

        if (ccn_iribu_mgmt_create_interface_stmt(
                num_interfaces, interfaceifndx, interfacedev, interfacedevtype,
                interfacereflect, interfaceaddr, stmt, stmt + stmt_length, &len3)) {
            goto Bail;
        }

        if (ccn_iribu_mgmt_create_faces_stmt(num_faces, faceid, facenext, faceprev,
                                             faceifndx, faceflags, facetype, facepeer,
                                             facefrag, stmt, stmt + stmt_length, &len3)) {
            goto Bail;
        }

        if (ccn_iribu_mgmt_create_fwds_stmt(num_fwds, fwd, fwdnext, fwdface, fwdfaceid,
                                            suite, fwdprefixlen, fwdprefix, stmt,
                                            stmt + stmt_length, &len3)) {
            goto Bail;
        }

        if (ccn_iribu_mgmt_create_interest_stmt(
                num_interests, interest, interestnext, interestprev, interestlast,
                interestmin, interestmax, interestretries, interestpublisher,
                interestprefixlen, interestprefix, stmt, stmt + stmt_length, &len3)) {
            goto Bail;
        }

        if (ccn_iribu_mgmt_create_content_stmt(
                num_contents, content, contentnext, contentprev, contentlast_use,
                contentserved_cnt, ccontents, cprefix, stmt, stmt + stmt_length, &len3)) {
            goto Bail;
        }
    }
    ...
    if (ccn_iribu_ccnb_mkBlob(out + len, out + object_length, CCN_DTAG_CONTENT,
                              CCN_TT_DTAG,    // content
                              (char *) stmt, len3, &len)) {
        goto Bail;
    }
...
int8_t ccn_iribu_mgmt_newface(struct ccn_iribu_relay_s *ccn_iribu,
                              struct ccn_iribu_buf_s *orig,
                              struct ccn_iribu_prefix_s *prefix,
                              struct ccn_iribu_face_s *from)
{
...
#        if !(defined(__FreeBSD__) || defined(__APPLE__))
    if (macsrc && host && port) {
        ...
        if (ccn_iribu_mgmt_parse_eth_address(su.linklayer.sll_addr,
                                             (const char *) host)) {
            goto SoftBail;
        }
...
#    endif
    ...
    if (f) {
        ...
        f->flags = flagval & (CCN_IRIBU_FACE_FLAGS_STATIC | CCN_IRIBU_FACE_FLAGS_REFLECT);
...
#    ifdef USE_IPV4
        if (ip4src != NULL) {
            DEBUGMSG(TRACE,
                     "  newface request for (macsrc=%s ip4src=%s proto=%s host=%s "
                     "port=%s frag=%s flags=%s) failed or was ignored\n",
...
#    ifdef USE_IPV6
        if (ip6src != NULL) {
            DEBUGMSG(TRACE,
                     "  newface request for (macsrc=%s ip6src=%s proto=%s host=%s "
                     "port=%s frag=%s flags=%s) failed or was ignored\n",
...
SoftBail:
    ...
    if (ccn_iribu_ccnb_mkHeader(out_buf, out_buf + OUT_BUF_SIZE, CCN_DTAG_NAME,
                                CCN_TT_DTAG, &len)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, "ccnx", &len)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, "", &len)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, "newface", &len)) {
        goto Bail;
    }
    ...
    if (ccn_iribu_ccnb_mkHeader(faceinst_buf, faceinst_buf + FACEINST_BUF_SIZE,
                                CCN_DTAG_FACEINSTANCE, CCN_TT_DTAG, &len3)) {
        goto Bail;
    }
    ...
    if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, (char *) retstr, &len3)) {
        goto Bail;
    }
    if (macsrc) {
        if (ccn_iribu_ccnb_mkStrBlob(
                faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                CCN_IRIBU_DTAG_MACSRC, CCN_TT_DTAG, (char *) macsrc, &len3)) {
            goto Bail;
        }
    }
    if (ip4src) {
        if (ccn_iribu_ccnb_mkStrBlob(
                faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                CCN_IRIBU_DTAG_IP4SRC, CCN_TT_DTAG, (char *) ip4src, &len3)) {
            goto Bail;
        }
        if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3,
                                     faceinst_buf + FACEINST_BUF_SIZE, CCN_DTAG_IPPROTO,
                                     CCN_TT_DTAG, "17", &len3)) {
            goto Bail;
        }
    }
    if (ip6src) {
        if (ccn_iribu_ccnb_mkStrBlob(
                faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                CCN_IRIBU_DTAG_IP6SRC, CCN_TT_DTAG, (char *) ip6src, &len3)) {
            goto Bail;
        }
        if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3,
                                     faceinst_buf + FACEINST_BUF_SIZE, CCN_DTAG_IPPROTO,
    ...
    if (host) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3,
                                     faceinst_buf + FACEINST_BUF_SIZE, CCN_DTAG_HOST,
                                     CCN_TT_DTAG, (char *) host, &len3)) {
            goto Bail;
        }
    }
    if (port) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3,
                                     faceinst_buf + FACEINST_BUF_SIZE, CCN_DTAG_PORT,
                                     CCN_TT_DTAG, (char *) port, &len3)) {
            goto Bail;
        }
    }
    /*
    if (frag) {
        if (ccn_iribu_ccnb_mkStrBlob(faceinst+len3, faceinst_buf + FACEINST_BUF_SIZE,
    CCN_IRIBU_DTAG_FRAG, CCN_TT_DTAG, frag, &len3)) { goto Bail;
        }
    }
    */
    if (flags) {
        if (ccn_iribu_ccnb_mkStrBlob(
                faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                CCN_IRIBU_DTAG_FACEFLAGS, CCN_TT_DTAG, (char *) flags, &len3)) {
            goto Bail;
        }
    }
    if (f) {
        ...
        if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3,
                                     faceinst_buf + FACEINST_BUF_SIZE, CCN_DTAG_FACEID,
                                     CCN_TT_DTAG, (char *) faceidstr, &len3)) {
            goto Bail;
        }
    }
    ...
    if (ccn_iribu_ccnb_mkBlob(out_buf + len, out_buf + OUT_BUF_SIZE, CCN_DTAG_CONTENT,
                              CCN_TT_DTAG,    // content
...
int8_t ccn_iribu_mgmt_setfrag(struct ccn_iribu_relay_s *ccn_iribu,
                              struct ccn_iribu_buf_s *orig,
                              struct ccn_iribu_prefix_s *prefix,
                              struct ccn_iribu_face_s *from)
{
...
#    endif
    ...
    Error:
        DEBUGMSG(
            TRACE,
            "  setfrag request for (faceid=%s frag=%s mtu=%s) failed or was ignored\n",
            faceid, frag, mtu);
    }

SoftBail:

    if (ccn_iribu_ccnb_mkHeader(out_buf + len, out_buf + OUT_BUF_SIZE, CCN_DTAG_NAME,
                                CCN_TT_DTAG, &len)) {    // name
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, "ccnx", &len)) {
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, "setfrag", &len)) {
        goto Bail;
    }
    if (len + 1 >= OUT_BUF_SIZE) {
        goto Bail;
    ...
    if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, cp, &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                                 CCN_DTAG_FACEID, CCN_TT_DTAG, (char *) faceid, &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                                 CCN_IRIBU_DTAG_FRAG, CCN_TT_DTAG, (char *) frag,
                                 &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                                 CCN_IRIBU_DTAG_MTU, CCN_TT_DTAG, (char *) mtu, &len3)) {
        goto Bail;
    }
...
int8_t ccn_iribu_mgmt_destroyface(struct ccn_iribu_relay_s *ccn_iribu,
                                  struct ccn_iribu_buf_s *orig,
                                  struct ccn_iribu_prefix_s *prefix,
                                  struct ccn_iribu_face_s *from)
{
...
SoftBail:
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, "destroyface", &len)) {
        goto Bail;
    }
    if (len + 1 >= OUT_BUF_SIZE) {
        goto Bail;
...
int8_t ccn_iribu_mgmt_newdev(struct ccn_iribu_relay_s *ccn_iribu,
                             struct ccn_iribu_buf_s *orig,
                             struct ccn_iribu_prefix_s *prefix,
                             struct ccn_iribu_face_s *from)
{
...
#    if defined(USE_LINKLAYER) && \
        (defined(CCN_IRIBU_UNIX) || defined(CCN_IRIBU_LINUXKERNEL))
    if (devname && port) {
...
#        endif
        ...
        if (ccn_iribu->defaultInterfaceScheduler) {
            i->sched =
                ccn_iribu->defaultInterfaceScheduler(ccn_iribu, ccn_iribu_interface_CTS);
        }
        ccn_iribu->ifcount++;
...
#    ifdef USE_IPV4
    if (ip4src) {
        DEBUGMSG(TRACE,
                 "  newdevice request for (namedev=%s ip4src=%s port=%s frag=%s) failed "
                 "or was ignored\n",
...
#    ifdef USE_IPV6
    if (ip6src) {
        DEBUGMSG(TRACE,
                 "  newdevice request for (namedev=%s ip6src=%s port=%s frag=%s) failed "
                 "or was ignored\n",
...
SoftBail:
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, "newdev", &len)) {
        goto Bail;
    }
    if (len + 1 >= OUT_BUF_SIZE) {
        goto Bail;
    ...
    // prepare DEVINSTANCE
    if (ccn_iribu_ccnb_mkHeader(faceinst_buf, faceinst_buf + FACEINST_BUF_SIZE,
                                CCN_IRIBU_DTAG_DEVINSTANCE, CCN_TT_DTAG, &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, cp, &len3)) {
    ...
    if (devname) {
        if (ccn_iribu_ccnb_mkStrBlob(
                faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                CCN_IRIBU_DTAG_DEVNAME, CCN_TT_DTAG, (char *) devname, &len3)) {
            goto Bail;
        }
    }
    ...
    if (devname && port) {
        ...
        if (frag) {
            if (ccn_iribu_ccnb_mkStrBlob(
                    faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                    CCN_IRIBU_DTAG_FRAG, CCN_TT_DTAG, (char *) frag, &len3)) {
                goto Bail;
            }
        }
        if (flags) {
            if (ccn_iribu_ccnb_mkStrBlob(
                    faceinst_buf + len3, faceinst_buf + FACEINST_BUF_SIZE,
                    CCN_IRIBU_DTAG_DEVFLAGS, CCN_TT_DTAG, (char *) flags, &len3)) {
                goto Bail;
            }
        }
        if (len3 + 1 >= FACEINST_BUF_SIZE) {
...
int8_t ccn_iribu_mgmt_destroydev(struct ccn_iribu_relay_s *ccn_iribu,
                                 struct ccn_iribu_buf_s *orig,
                                 struct ccn_iribu_prefix_s *prefix,
                                 struct ccn_iribu_face_s *from)
{
    ...
    /*ANSWER*/
    ccn_iribu_mgmt_return_ccn_msg(ccn_iribu, orig, prefix, from, "mgmt_destroy",
                                  "mgmt_destroydev not implemented yet");
...
int8_t ccn_iribu_mgmt_echo(struct ccn_iribu_relay_s *ccn_iribu,
                           struct ccn_iribu_buf_s *orig,
                           struct ccn_iribu_prefix_s *prefix,
                           struct ccn_iribu_face_s *from)
{
    ...
    p->comp    = (unsigned char **) ccn_iribu_malloc(CCN_IRIBU_MAX_NAME_COMP *
...
SoftBail:
    /*ANSWER*/
    if (!action || !p) {
        if (ccn_iribu_mgmt_return_ccn_msg(ccn_iribu, orig, prefix, from, "echoserver",
                                          cp)) {
            goto Bail;
        }
    }
    ...
    if (ccn_iribu_ccnb_mkStrBlob(out_buf + len, out_buf + OUT_BUF_SIZE,
                                 CCN_DTAG_COMPONENT, CCN_TT_DTAG, (char *) action,
                                 &len)) {
        goto Bail;
    }
    if (len + 1 >= OUT_BUF_SIZE) {
    ...
    // prepare FWDENTRY
    if (ccn_iribu_ccnb_mkHeader(fwdentry_buf, fwdentry_buf + FWDENTRY_BUF_SIZE,
                                CCN_IRIBU_DTAG_PREFIX, CCN_TT_DTAG, &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(fwdentry_buf + len3, fwdentry_buf + FWDENTRY_BUF_SIZE,
                                 CCN_DTAG_ACTION, CCN_TT_DTAG, cp, &len3)) {
        goto Bail;
    }
    if (ccn_iribu_ccnb_mkStrBlob(fwdentry_buf + len3, fwdentry_buf + FWDENTRY_BUF_SIZE,
                                 CCN_DTAG_NAME, CCN_TT_DTAG,
                                 ccn_iribu_prefix_to_str(p, s, CCN_IRIBU_MAX_PREFIX_SIZE,
                                                         &len3))) {    // prefix
        goto Bail;
    }

    //    len3 += ccn_iribu_ccnb_mkStrBlob(fwdentry_buf+len3, CCN_DTAG_FACEID,
    //    CCN_TT_DTAG, (char*) faceid);
    memset(h, 0, sizeof(h));
    snprintf((char *) h, sizeof(h), "%d", (int) suite[0]);
    if (ccn_iribu_ccnb_mkStrBlob(fwdentry_buf + len3, fwdentry_buf + FWDENTRY_BUF_SIZE,
                                 CCN_IRIBU_DTAG_SUITE, CCN_TT_DTAG, (char *) h, &len3)) {
        goto Bail;
    }
...
int8_t ccn_iribu_mgmt_prefixreg(struct ccn_iribu_relay_s *ccn_iribu,
                                struct ccn_iribu_buf_s *orig,
                                struct ccn_iribu_prefix_s *prefix,
                                struct ccn_iribu_face_s *from)
{
    ...
    if (faceid && p->compcnt > 0) {
        ...
        DEBUGMSG(TRACE, "mgmt: adding prefix %s to faceid=%s, suite=%s\n",
                 ccn_iribu_prefix_to_str(p, s, CCN_IRIBU_MAX_PREFIX_SIZE), faceid,
                 ccn_iribu_suite2str(suite[0]));
...
int8_t ccn_iribu_mgmt_addcacheobject(struct ccn_iribu_relay_s *ccn_iribu,
                                     struct ccn_iribu_buf_s *orig,
                                     struct ccn_iribu_prefix_s *prefix,
                                     struct ccn_iribu_face_s *from)
{
    ...
    prefix_new = ccn_iribu_URItoPrefix((char *) components, CCN_IRIBU_SUITE_CCNB,
                                       chunkflag ? &chunknum : NULL);
    ...
    DEBUGMSG(TRACE, "  mgmt: adding object %s to cache (suite=%s)\n",
             ccn_iribu_prefix_to_str(ccn_iribu_prefix_dup(prefix_new), s,
                                     CCN_IRIBU_MAX_PREFIX_SIZE),
             ccn_iribu_suite2str(suite));
    ...
    ret = snprintf((char *) h, 300,
                   "received add to cache request, inizializing callback for %s",
                   ccn_iribu_prefix_to_str(prefix_new, s, CCN_IRIBU_MAX_PREFIX_SIZE));
    if (ret < 0 || (unsigned) ret >= 300) {
        goto Bail;
    }
    if (ccn_iribu_mgmt_return_ccn_msg(ccn_iribu, orig, prefix, from, "addcacheobject",
                                      (char *) h)) {
        ccn_iribu_free(h);
        goto Bail;
    }
...
int8_t ccn_iribu_mgmt_removecacheobject(struct ccn_iribu_relay_s *ccn_iribu,
                                        struct ccn_iribu_buf_s *orig,
                                        struct ccn_iribu_prefix_s *prefix,
                                        struct ccn_iribu_face_s *from)
{
...
SoftBail:
    // send answer
    if (ccn_iribu_mgmt_return_ccn_msg(ccn_iribu, orig, prefix, from, "removecacheobject",
                                      answer)) {
        goto Bail;
    }
...
#    ifdef USE_SIGNATURES
int8_t ccn_iribu_mgmt_validate_signature(struct ccn_iribu_relay_s *ccn_iribu,
                                         struct ccn_iribu_buf_s *orig,
                                         struct ccn_iribu_prefix_s *prefix,
                                         struct ccn_iribu_face_s *from, char *cmd)
{
    ...
    if (ccn_iribu_crypto_verify(ccn_iribu, data, datalen, (char *) sig, siglen,
                                "ccn_iribu_mgmt_crypto", from->faceid)) {
        goto Bail;
    }
...
int8_t ccn_iribu_mgmt_handle(struct ccn_iribu_relay_s *ccn_iribu,
                             struct ccn_iribu_buf_s *orig,
                             struct ccn_iribu_prefix_s *prefix,
                             struct ccn_iribu_face_s *from, char *cmd, int8_t verified)
{
