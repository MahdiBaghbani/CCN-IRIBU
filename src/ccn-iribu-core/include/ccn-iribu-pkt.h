...
#    define CCN_IRIBU_PKT_FRAG_BEGIN \
        0x04    // see also CCN_IRIBU_DATA_FRAG_FLAG_FIRST etc
...
#    ifdef USE_SUITE_NDNTLV
    struct ccn_iribu_ndntlv_interest_opts_s
        ndntlv; /**< options for NDN Interest messages */
#    endif
...
struct ccn_iribu_pktdetail_ndntlv_s {
    ...
    /* Data */
    uint64_t freshnessperiod; /**< defines how long a node has to wait (after the arrival
                                 of this data before) marking it “non-fresh” */
};
...
/**
 * @brief Create a component for a pkt data structure (CCNTLV and CISTLV need special
 * component start)
 *
