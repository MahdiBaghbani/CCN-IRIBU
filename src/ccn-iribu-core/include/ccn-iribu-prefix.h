...
#ifndef CCN_IRIBU_PREFIX_H
#    define CCN_IRIBU_PREFIX_H
...
/**
 * @brief Create a new CCN_IRIBU_Prefix datastructure
 *
...
/**
 * @brief Frees CCN_IRIBU_Prefix datastructure
 *
 * @param[in] prefix       Prefix to be freed
 */
void ccn_iribu_prefix_free(struct ccn_iribu_prefix_s *prefix);

/**
 * @brief Duplicates CCN_IRIBU_Prefix datastructure
 *
 ...
 * @return      0 if full match (mode = CMP_EXACT) or no components match (mode =
 * CMP_MATCH)
...
// int
// ccn_iribu_URItoComponents(char **compVector, unsigned int *compLens, char *uri);

/**
 * @brief Take a URI and transform it into a CCN_IRIBU_Prefix datastructure
 *
 ...
 * @param[in] chunknum  If prefix is part of a chunked data file, this contains the
 * chunknumber
 *
 * @return The prefix datastruct that was created
 */
struct ccn_iribu_prefix_s *ccn_iribu_URItoPrefix(char *uri, int suite,
                                                 uint32_t *chunknum);
 ...
 * @param[in] uri               The uri that should be transformed, components separated
 * by '/'
 *
 ...
 * @param[in]  ccntlv_skip          Flag to enable skipping of CCN_IRIBU_TLV fields
 ...
 *       @ref ccn_iribu_prefix_to_str_detailed
 *
 * @param[in] pr                The prefix that should be transformed
 * @param[in] ccntlv_skip       Flag to enable skipping of CCN_IRIBU_TLV fields
 * @param[in] escape_components Flag to enable escaping components
...
#        define ccn_iribu_prefix_to_path(P) ccn_iribu_prefix_to_path_detailed(P, 1, 0, 0)
#        define ccn_iribu_prefix_to_str(P, buf, buflen) \
            ccn_iribu_prefix_to_str_detailed(P, 1, 0, 0, buf, buflen)
#    else
/**
...
/**
 * @brief Transforms a Prefix into a URI, separated bei '/', that contains debug
 * information
 *
