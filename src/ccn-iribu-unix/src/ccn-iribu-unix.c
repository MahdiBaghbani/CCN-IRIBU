...
void ccn_iribu_relay_udp(struct ccn_iribu_relay_s *relay, int32_t sport, int af,
                         int suite)
{
...
#    endif
    ...
    if (relay->defaultInterfaceScheduler)
        i->sched = relay->defaultInterfaceScheduler(relay, ccn_iribu_interface_CTS);
}
#endif
...
#ifdef USE_IPV4
    case AF_INET:
        rc = sendto(ifc->sock, buf->data, buf->datalen, 0, (struct sockaddr *) &dest->ip4,
                    sizeof(struct sockaddr_in));
        DEBUGMSG(DEBUG, "udp sendto %s/%d returned %zd\n", inet_ntoa(dest->ip4.sin_addr),
                 ntohs(dest->ip4.sin_port), rc);
        /*
        {
...
#ifdef USE_IPV6
    case AF_INET6:
        rc = sendto(ifc->sock, buf->data, buf->datalen, 0, (struct sockaddr *) &dest->ip6,
...
#ifdef USE_LINKLAYER
    case AF_PACKET:
        rc = ccn_iribu_eth_sendto(ifc->sock, dest->linklayer.sll_addr,
                                  ifc->addr.linklayer.sll_addr, buf->data, buf->datalen);
...
#ifdef USE_UNIXSOCKET
    case AF_UNIX:
        rc = sendto(ifc->sock, buf->data, buf->datalen, 0, (struct sockaddr *) &dest->ux,
                            ...
                            int max_cache_entries, char *crypto_face_path)
{
    ...
    if (ethdev) {
        ...
        if (i->sock >= 0) {
            ...
            if (relay->defaultInterfaceScheduler)
                i->sched =
                    relay->defaultInterfaceScheduler(relay, ccn_iribu_interface_CTS);
        } else
            ...
            if (FD_ISSET(ccn_iribu->ifs[i].sock, &readfs)) {
                ...
                if ((recvlen = recvfrom(ccn_iribu->ifs[i].sock, buf, sizeof(buf), 0,
                    ...
                    if (0) {
                        ...
                        ccn_iribu_core_RX(ccn_iribu, i, buf, len, &src_addr.sa,
                                          sizeof(src_addr.ip4));
                    }
#endif
#ifdef USE_IPV6
